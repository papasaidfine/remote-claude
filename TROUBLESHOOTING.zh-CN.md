...
### Linux：ListenAddress 不生效（Ubuntu 23.10+）

较新的 Ubuntu 用 systemd **socket activation**（`ssh.socket`）拉起 sshd；这种模式下监听地址由 socket unit 决定，sshd_config 里的 `ListenAddress` 会被忽略。两种处理方式，切回 service：

```bash
sudo systemctl disable --now ssh.socket && sudo systemctl enable --now ssh.service
```

或覆盖 socket 配置：`sudo systemctl edit ssh.socket`，写入 `ListenStream=` 再写 `ListenStream=127.0.0.1:22`。bootstrap 脚本检测到这种情况时会给出警告。
