...
# Starts a private xray (dokodemo-door -> DestHost:DestPort) on a free port,
# bridges stdio to it, and guarantees xray dies with this process via a
# kill-on-close Job Object (survives ssh's TerminateProcess teardown).
param(
...
$conf = Get-Content -Raw (Join-Path $RcDir 'xray.json')
